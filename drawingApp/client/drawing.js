...
Canvas = function () {
  ...
  self.draw = function(data, data_circle, data_rectangle) {
    if (data.length < 1 && data_circle.length < 1 && data_rectangle.length < 1) {
      self.clear();
      return;
    }
    if (svg) {
	  ...
	  svg.selectAll('circle').data(data_circle, function(d) { return d._id; })
	   ...
	   svg.selectAll('rect').data(data_rectangle, function(d) { return d._id; })
       .enter().append('rect')
